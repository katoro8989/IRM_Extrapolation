...
from utils.training_utils import criterion, penalty_v1, penalty_v1b, mean_accuracy, penalty_stationary, penalty_v0, calc_ece_ace, get_maxprob_and_onehot, init_config, penalty_fishr

def train(
        model, args, device, train_loaders, optimizer, scheduler, epoch, ema_per_domain
):
    ...
    while 1:
        ...
        for env_num, data_iter in enumerate(data_iters):
            ...
            envs[env_num]["penalty_v1"] = penalty_v1(logits, labels)
            envs[env_num]["penalty_v0"] = torch.tensor(0.0)
            envs[env_num]["penalty_stationary"] = torch.tensor(0.0)
        ...
        len_minibatches = [len(loader) for loader in train_loaders]
        all_logits = torch.cat(all_logits, dim=0)
        all_labels = torch.cat(all_labels, dim=0)
        fishr_penalty = penalty_fishr(all_logits, all_labels, len_minibatches, optimizer, model, len(data_iters), ema_per_domain)
